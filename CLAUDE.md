...
**Duels (Phase 4a, `docs/SPEC_multiplayer_pvp.md`).** A visit with `mode = "duel"` is a hostile 1v1 with gold in escrow. It is the same engine and the same `sc`/`s` settlement as co-op, plus three things co-op does not have: each round is a **commit, then reveal, then apply** protocol so neither player sees the other's choice first; the RNG is **reseeded per round** from both revealed salts, so nobody can read ahead; and bumping a hostile participant **attacks** instead of being blocked. The winner is whoever is left when at most one participant is active — a gate exit is a concession, not an escape — and is banked as survived without reaching a gate. Duel mode is the only thing that enables any of those paths, so a co-op or solo replay is byte-identical to before it existed. The backend is complete; the frontend mirror is not (see ROADMAP).
...
**Version handshake.** `rules.hpp` holds two constants that `getcurrentstate` exposes as `version: {rules, banking}`. Bump `RULES_VERSION` for anything the replay depends on (a draw, an action, a seed derivation, an encoding, the commitment preimage) — a client that does not match exactly must refuse to start a run, because its local run would be rejected at settlement after the player had already played it. Bump `BANKING_VERSION` for what settlement awards (pools, pot, XP, survival heal, penalties, timeouts) — the client's engine is unaffected, so it keeps playing but should stop projecting those numbers in its HUD. Bump exactly one, in the same commit as the change, and add a line to the history comment in `rules.hpp`.
