...
**Move pipeline.** `moveparser.cpp` validates JSON shape and calls virtual `Process*` hooks; `moveprocessor.cpp` overrides them to mutate the database. Moves use single-letter keys (`r` register, `d` discover, `t` travel, `ec`/`xc` enter/exit channel, `gw` gate-walk, `ui`/`eq`/`uq` items, `as` stats — full table in README.md). `pending.cpp` mirrors a subset for mempool tracking. Segment/visit IDs come from simple deterministic counters threaded through `MoveProcessor`.
